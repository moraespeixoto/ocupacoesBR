...
# o bloco de codigo da home tem cerca de 600px: a saida precisa caber nele
options(width = 62)
library(ocupacoesBR)
versao <- as.character(utils::packageVersion("ocupacoesBR"))
n_codigos <- nrow(tse_isco)
ano_min <- min(tse_ocupacao_rotulos$de)
ano_max <- max(tse_ocupacao_rotulos$ate)
cods_2026 <- unique(tse_ocupacao_rotulos$cod_tse[tse_ocupacao_rotulos$ate == ano_max])
n_2026 <- length(cods_2026)
# um recorte com os nomes de coluna que o TSE usa, para o exemplo abaixo
dados <- data.frame(
...
<span class="ocb-badge"><span class="ocb-badge-k">cadastro TSE</span><span class="ocb-badge-v">`r ano_min`–`r ano_max`</span></span>
...
<h3>TSE</h3>
<p>O código de ocupação das candidaturas, de `r ano_min` a `r ano_max`. A única ponte autoral do pacote, com régua de qualidade em cada linha.</p>
...
<h3>A safra de `r ano_max`, na régua</h3>
<p>Nenhum código novo: os `r n_2026` códigos declarados em `r ano_max` são subconjunto dos `r n_codigos` que o pacote já cobria.</p>
</a>
...
```{r citacao, echo = FALSE, comment = ""}
print(utils::citation("ocupacoesBR")[[1]], style = "text")
```
...
<a href="news/index.html">Novidades (`r versao`)</a>
...
<a href="articles/safra-2026.html">Safra `r ano_max`</a>
</div>
<div>
